...
import { alias, sort, or } from "@ember/object/computed";
...
export default ItemBaseController.extend({
  ...
  isOfferVanished: or("model.isDeleted", "model.isDeleting"),

  showDeleteError: observer("model", "isOfferVanished", function() {
  ...
  hasActiveGGVOrder: alias("model.delivery.gogovanOrder.isActive"),

  offerAndItems: computed("model", "items.@each.state", function() {
  ...
  displayHomeLink: computed("offers.@each.state", function() {
    return (
      this.get("offers")
        .rejectBy("state", "draft")
        .get("length") > 0
