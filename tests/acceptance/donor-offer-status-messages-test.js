...
module("Donor: Display Offer Status", {
  beforeEach: function() {
    ...
    offer8 = FactoryGuy.make("offer_with_items", {
      state: "under_review",
    ...
    offer9 = FactoryGuy.make("offer_with_items", {
      state: "scheduled",
      delivery: delivery3
    });
...
test("Display offer status for offer under review", function() {
  ...
  andThen(function() {
    ...
    equal(
      status.indexOf("Your offer is being reviewed by " + reviewerName + ".") >=
        0,
      true
    );
...
test("Display offer status for reviewed offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim(find(".status-message").text()),
      "Review complete! Please arrange transport."
    );
  });
});
...
test("Display offer status for scheduled offer: Gogovan", function() {
  ...
  andThen(function() {
    ...
    equal(
      ...
      "Van confirmed Afternoon, 2pm-4pm, Mon 1st Dec"
    );
  });
});
...
test("Display offer status for closed offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim(find(".status-message").text()),
      "Offer closed. No items needed, Sorry."
    );
  });
});
...
test("Display offer status for received offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim(find(".status-message").text()).indexOf("Offer received") >= 0,
      true
    );
  });
});
