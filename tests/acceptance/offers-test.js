...
module("Offer Index View", {
  beforeEach: function() {
    ...
    del3 = FactoryGuy.make("delivery", {
      deliveryType: "Gogovan",
      gogovanOrder: ggvOrder1
    });
    ...
    del4 = FactoryGuy.make("delivery", {
      deliveryType: "Gogovan",
      gogovanOrder: ggvOrder2
    });
    offer7 = FactoryGuy.make("offer", {
      state: "scheduled",
      delivery: del4,
      createdBy: user
    });
...
test("Offers list & link to add items", function(assert) {
  ...
  andThen(function() {
    ...
    var checkLink = function(offerId, expectedText) {
      ...
      var foundText = $("a[href='" + url + "'] h3").text();
      assert.ok(
        foundText.indexOf(expectedText) > -1,
        "Found: " + foundText + " => Expected: " + expectedText
      );
    };
