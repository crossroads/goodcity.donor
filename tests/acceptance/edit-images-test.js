...
module("Edit Images", {
  beforeEach: function() {
    ...
    $.mockjax({
      url: "/api/v1/images/generate_signatur*",
      responseText: {
        api_key: "1111",
    ...
    edit_images_url =
      "/offers/" + offer.id + "/items/" + item.id + "/edit_images";
  },
...
testSkip("Add Image: display previously added images", function() {
  ...
  andThen(function() {
    ...
    andThen(function() {
      equal(
        currentURL(),
        "/offers/" + offer.id + "/items/" + item.id + "/edit"
      );
      ...
      andThen(function() {
        // preview-image
        equal(
          find("#main-image > div").css("background-image"),
          "url(" + img1.get("imageUrl") + ")"
        );
        ...
        // favourite-image
        equal(
          find("#photo-list .fa-star:not(.hidden)")
            .prev()
            .attr("src"),
          img1.get("thumbImageUrl")
        );
      });
    });
  });
