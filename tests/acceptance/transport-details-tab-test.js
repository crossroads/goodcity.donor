...
module("Display Transport Details", {
  beforeEach: function() {
    ...
    App.__container__.lookup("service:session").set("isAdmin", false);
    ...
    delivery1 = FactoryGuy.make("delivery", {
      ...
      gogovanOrder: ggv_order1
    });
    offer5 = FactoryGuy.make("offer", {
      state: "scheduled",
      delivery: delivery1
    });
    ...
    ggv_order2 = FactoryGuy.make("gogovan_active_order", {
      driverMobile: "12345678"
    });
    delivery2 = FactoryGuy.make("delivery", {
      deliveryType: "Gogovan",
      gogovanOrder: ggv_order2
    });
    offer6 = FactoryGuy.make("offer", {
      state: "scheduled",
      delivery: delivery2
    });
    item6 = FactoryGuy.make("item", { state: "accepted", offer: offer6 });

    ggv_order3 = FactoryGuy.make("gogovan_completed_order", {
      driverMobile: "12345678"
    });
    delivery3 = FactoryGuy.make("delivery", {
      deliveryType: "Gogovan",
      gogovanOrder: ggv_order3
    });
...
test("for submitted offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim($(".wait_transport").text()),
      "Transport arrangements can be made once the review is complete."
    );
  });
});
...
test("for under review offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim($(".transport-content .row:first").text()),
      "Items accepted so far (1)"
    );
...
test("for reviewed offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $(".transport-content .row:first a").attr("href"),
      "/offers/" + offer3.id + "/plan_delivery"
    );
    equal(
      $.trim($(".transport-content .row:eq(1)").text()),
...
test("for scheduled offer", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim($(".transport-content .row:eq(6)").text()),
      "Accepted items to be transported"
    );
    equal($(".items_list img").length, 1);
...
test("for scheduled offer with pending GGV order state", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim($(".delivery-details .row:eq(0)").text()).indexOf(
    ...
    equal(
      $.trim($(".delivery-details .row:eq(0)").text()).indexOf(
        "Driver & vehicle details will appear here once a driver accepts your booking."
      ) > 0,
      true
    );

    equal(
      $.trim($(".transport-content .row:eq(4)").text()),
      "Accepted items to be transported"
    );
    equal($(".items_list img").length, 1);
    equal($(".transport-buttons a").length, 2);
...
test("for scheduled offer with active GGV order state", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim($(".transport-content .row:eq(0)").text()).indexOf(
        "Booking Confirmed"
      ) >= 0,
      true
    );
    ...
    // driver name
    equal(
      $.trim($(".transport-content .row:eq(2)").text()).indexOf(
        ggv_order2.get("driverName")
      ) > 0,
      true
    );
    ...
    // driver mobile
    equal(
      $.trim($(".transport-content .row:eq(3)").text()).indexOf("1234 5678") >
        0,
      true
    );
    ...
    // driver License
    equal(
      $.trim($(".transport-content .row:eq(4)").text()).indexOf(
        ggv_order2.get("driverLicense")
      ) > 0,
      true
    );

    equal(
      $.trim($(".transport-content .row:eq(5)").text()).indexOf(
        ggv_order2.get("price")
      ) > 0,
      true
    );

    equal(
      $.trim($(".transport-content .row:eq(8)").text()),
      "Accepted items to be transported"
    );
    equal($(".items_list img").length, 1);
    equal($(".transport-buttons a").length, 2);
...
test("for scheduled offer with completed GGV order state", function() {
  ...
  andThen(function() {
    ...
    equal(
      $.trim($(".transport-content .row:eq(0)").text()).indexOf("Picked up") >=
        0,
      true
    );
...
test("cancel booking of scheduled offer with active GGV order state", function() {
  ...
  andThen(function() {
    ...
    andThen(function() {
      equal(
        currentURL(),
        "/offers/" + offer6.id + "/delivery/" + delivery2.id + "/cancel_booking"
      );
    });
  });
});
